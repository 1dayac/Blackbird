...
static inline void mi_atomic_maxi64(volatile _Atomic(int64_t)* p, int64_t val) {
  MI_USING_STD
  int64_t prev_value =  (int64_t)mi_atomic_read((volatile _Atomic(uintptr_t)*)p);
  // Cannot use mi_atomic_cas_weak as we need to update prev_value in CAS loop
  while (prev_value < val) {
      prev_value = MI_64(_InterlockedCompareExchange)((volatile msc_intptr_t*)p,
                                                      (msc_intptr_t)val, (msc_intptr_t)prev_value);
  }
}
...
static inline void mi_atomic_maxi64(volatile int64_t* p, int64_t val) {
  MI_USING_STD
  int64_t prev_value =  (int64_t)mi_atomic_read((volatile _Atomic(uintptr_t)*)p);
  // Cannot use mi_atomic_cas_weak as we need to update prev_value in CAS loop
  while (prev_value < val &&
         !atomic_compare_exchange_weak_explicit((volatile _Atomic(int64_t)*)p,
                                                &prev_value, val,
